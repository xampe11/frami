...
contract DeployPhase1 is Script {
    ...
    function loadConfiguration() internal view returns (Phase1Config memory) {
        ...
        console.log("   Min Staking Period:", config.minimumStakingPeriod / 86_400, "days");
    ...
    function validateConfiguration(Phase1Config memory config) internal pure {
        ...
        require(config.maxSupply > 0 && config.maxSupply <= 100_000, "Invalid max supply");
        require(config.price > 0, "Price must be greater than 0");
        require(config.platformFeeDistributionPercentage <= 10_000, "Fee distribution percentage too high");
        require(config.daoTokenAllocationPercentage <= 10_000, "DAO allocation percentage too high");
    ...
    function deployContracts(Phase1Config memory config) internal returns (DeploymentResult memory) {
        ...
        bytes memory platformRegistryData = abi.encodeWithSelector(
            PlatformRegistry.initialize.selector, config.deployer, config.platformFeePercentage, config.treasury
        );
    ...
    function registerFounderNFTExtension(DeploymentResult memory result, Phase1Config memory /* config */ ) internal {
        uint256 deployerPrivateKey = vm.envUint("DEPLOYER_PRIVATE_KEY");
    ...
    function runIntegrationTests(DeploymentResult memory result, Phase1Config memory config) internal view {
        ...
        require(
            founderNFT.hasRole(founderNFT.PLATFORM_ROLE(), result.platformRegistryProxy), "Platform role not granted"
        );
    ...
    function logDeploymentSummary(DeploymentResult memory result, Phase1Config memory config) internal pure {
    ...
    function saveDeploymentData(DeploymentResult memory result, Phase1Config memory config) internal {
        ...
        string memory deploymentData = string(
            abi.encodePacked(
                ...
                ",\n",
                '  "deploymentBlock": ',
                vm.toString(result.deploymentBlock),
                ",\n",
                '  "deploymentTimestamp": ',
                vm.toString(result.deploymentTimestamp),
                ",\n",
                '  "deployer": "',
                vm.toString(config.deployer),
                '",\n',
                '  "treasury": "',
                vm.toString(config.treasury),
                '",\n',
                '  "contracts": {\n',
                '    "platformRegistry": {\n',
                '      "implementation": "',
                vm.toString(result.platformRegistryImpl),
                '",\n',
                '      "proxy": "',
                vm.toString(result.platformRegistryProxy),
                '"\n',
                "    },\n",
                '    "founderNFT": {\n',
                '      "implementation": "',
                vm.toString(result.founderNFTImpl),
                '",\n',
                '      "proxy": "',
                vm.toString(result.founderNFTProxy),
                '"\n',
                "    }\n",
                "  },\n",
                '  "configuration": {\n',
                '    "platformFeePercentage": ',
                vm.toString(config.platformFeePercentage),
                ",\n",
                '    "maxSupply": ',
                vm.toString(config.maxSupply),
                ",\n",
                '    "price": "',
                vm.toString(config.price),
                '",\n',
                '    "minimumStakingPeriod": ',
                vm.toString(config.minimumStakingPeriod),
                ",\n",
                '    "salesActive": ',
                ...
                ",\n",
                '    "daoTokenAllocationPercentage": ',
                vm.toString(config.daoTokenAllocationPercentage),
                "\n",
                "  }\n",
                "}"
            )
    ...
    function updateEnvFile(DeploymentResult memory result) internal {
        ...
        try vm.writeFile(
            ...
            string(
                abi.encodePacked(
                    "# Phase 1 Deployment Addresses (",
                    result.networkName,
                    ")\n",
                    "PLATFORM_REGISTRY_PROXY=",
                    vm.toString(result.platformRegistryProxy),
                    "\n",
                    "PLATFORM_REGISTRY_IMPL=",
                    vm.toString(result.platformRegistryImpl),
                    "\n",
                    "FOUNDER_NFT_PROXY=",
                    vm.toString(result.founderNFTProxy),
