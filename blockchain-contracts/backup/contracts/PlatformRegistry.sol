...
contract PlatformRegistry is Ownable, Pausable {
    ...
    function createProject(
        ...
        for (uint256 i = 0; i < _teamMembers.length; i++) {
            // Use low-level call to execute as msg.sender
            (bool success,) = projectAddress.call(abi.encodeWithSignature("addTeamMember(address)", _teamMembers[i]));
            require(success, "Failed to add team member");
        }
